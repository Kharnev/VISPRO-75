...
{
    public partial class FormResProfJasmineI : Form
    {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtUsername.Text != "")
                {
                    query = string.Format("select * from tbl_jasminei where namaJasmineI = '{0}'", txtUsername.Text);
                    ds.Clear();
                    koneksi.Open();
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        {
                            txtID.Text = kolom["ID_JasmineI"].ToString();
                            txtPassword.Text = kolom["NIMJasmineI"].ToString();
                            txtNama.Text = kolom["nokamJasmineI"].ToString();
                            CBLevel.Text = kolom["fakultasJasmineI"].ToString();
                    ...
                    else
                    {
                        ...
                        FormResProfJasmineI_Load(null, null);
                    }
        ...
        private void FormResProfJasmineI_Load(object sender, EventArgs e)
        {
            try
            {
                koneksi.Open();
                query = string.Format("select * from tbl_jasminei");
                perintah = new MySqlCommand(query, koneksi);
                adapter = new MySqlDataAdapter(perintah);
                perintah.ExecuteNonQuery();
                ds.Clear();
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtPassword.Text != "" && txtNama.Text != "" && txtUsername.Text != "" && txtID.Text != "")
                {
                    ...
                    query = string.Format("update tbl_jasminei set NIMJasmineI = '{0}', nokamJasmineI = '{1}', fakultasJasmineI = '{2}' where ID_JasmineI = '{3}'", txtPassword.Text, txtNama.Text, CBLevel.Text, txtID.Text);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtUsername.Text != "" && txtPassword.Text != "" && txtNama.Text != "")
                {
                    ...
                    query = string.Format("insert into tbl_jasminei values ('{0}','{1}','{2}','{3}','{4}');", txtID.Text, txtUsername.Text, txtPassword.Text, txtNama.Text, CBLevel.Text);
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtID.Text != "")
                {
                    ...
                    {
                        query = string.Format("Delete from tbl_jasminei where ID_JasmineI = '{0}'", txtID.Text);
                        ds.Clear();
                        koneksi.Open();
                        perintah = new MySqlCommand(query, koneksi);
                        adapter = new MySqlDataAdapter(perintah);
