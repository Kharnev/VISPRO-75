...
{
    public partial class FormResProfGuest : Form
    {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtUsername.Text != "")
                {
                    query = string.Format("select * from tbl_guest where namaGuest = '{0}'", txtUsername.Text);
                    ds.Clear();
                    koneksi.Open();
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        {
                            txtID.Text = kolom["ID_Guest"].ToString();
                            txtPassword.Text = kolom["NIMGuest"].ToString();
                            txtNama.Text = kolom["nokamGuest"].ToString();
                            CBLevel.Text = kolom["fakultasGuest"].ToString();
                    ...
                    else
                    {
                        ...
                        FormResProfGuest_Load(null, null);
                    }
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtPassword.Text != "" && txtNama.Text != "" && txtUsername.Text != "" && txtID.Text != "")
                {
                    ...
                    query = string.Format("update tbl_guest set NIMGuest = '{0}', nokamGuest = '{1}', fakultasGuest = '{2}' where ID_Guest = '{3}'", txtPassword.Text, txtNama.Text, CBLevel.Text, txtID.Text);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtUsername.Text != "" && txtPassword.Text != "" && txtNama.Text != "")
                {
                    ...
                    query = string.Format("insert into tbl_guest values ('{0}','{1}','{2}','{3}','{4}');", txtID.Text, txtUsername.Text, txtPassword.Text, txtNama.Text, CBLevel.Text);
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtID.Text != "")
                {
                    ...
                    {
                        query = string.Format("Delete from tbl_guest where ID_Guest = '{0}'", txtID.Text);
                        ds.Clear();
                        koneksi.Open();
                        perintah = new MySqlCommand(query, koneksi);
                        adapter = new MySqlDataAdapter(perintah);
        ...
        private void FormResProfGuest_Load(object sender, EventArgs e)
        {
            try
            {
                koneksi.Open();
                query = string.Format("select * from tbl_guest");
                perintah = new MySqlCommand(query, koneksi);
                adapter = new MySqlDataAdapter(perintah);
                perintah.ExecuteNonQuery();
                ds.Clear();
