...
{
    public partial class FormResProfBougenville : Form
    {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtUsername.Text != "")
                {
                    query = string.Format("select * from tbl_bougenville where namaBougenville = '{0}'", txtUsername.Text);
                    ds.Clear();
                    koneksi.Open();
                    ...
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        {
                            txtID.Text = kolom["ID_Bougenville"].ToString();
                            txtPassword.Text = kolom["NIMBougenville"].ToString();
                            txtNama.Text = kolom["nokamBougenville"].ToString();
                            CBLevel.Text = kolom["fakultasBougenville"].ToString();
                    ...
                    else
                    {
                        ...
                        FormResProfBougenville_Load(null, null);
                    }
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtPassword.Text != "" && txtNama.Text != "" && txtUsername.Text != "" && txtID.Text != "")
                {
                    ...
                    query = string.Format("update tbl_bougenville set NIMBougenville = '{0}', nokamBougenville = '{1}', fakultasBougenville = '{2}' where ID_Bougenville = '{3}'", txtPassword.Text, txtNama.Text, CBLevel.Text, txtID.Text);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtUsername.Text != "" && txtPassword.Text != "" && txtNama.Text != "")
                {
                    ...
                    query = string.Format("insert into tbl_bougenville values ('{0}','{1}','{2}','{3}','{4}');", txtID.Text, txtUsername.Text, txtPassword.Text, txtNama.Text, CBLevel.Text);
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtID.Text != "")
                {
                    ...
                    {
                        query = string.Format("Delete from tbl_bougenville where ID_Bougenville = '{0}'", txtID.Text);
                        ds.Clear();
                        koneksi.Open();
                        perintah = new MySqlCommand(query, koneksi);
                        adapter = new MySqlDataAdapter(perintah);
        ...
        private void FormResProfBougenville_Load(object sender, EventArgs e)
        {
            try
            {
                koneksi.Open();
                query = string.Format("select * from tbl_bougenville");
                perintah = new MySqlCommand(query, koneksi);
                adapter = new MySqlDataAdapter(perintah);
                perintah.ExecuteNonQuery();
                ds.Clear();
